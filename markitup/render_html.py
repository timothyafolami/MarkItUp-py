...
def render_html(doc_ir: ir.Document, theme) -> str:
    ...
    rules = _running_rules(theme)
    title = escape(doc_ir.title or "Document")
    return (
        "<!DOCTYPE html><html lang='en'><head><meta charset='utf-8'>"
        f"<title>{title}</title><style>{css}</style></head>"
        f"<body>{wm}{rules}{banner}{body}</body></html>"
    )


def _css_content(text: str) -> str:
    """Turn header/footer text with {page}/{pages} into a CSS `content` value."""
    out = []
...
def _running_rules(theme) -> str:
    """Full-width header/footer divider lines, fixed so they repeat every page."""
    out = []
    if theme.header and theme.header.rule:
        out.append("<div class='mk-hdr-rule'></div>")
    if theme.footer and theme.footer.rule:
        out.append("<div class='mk-ftr-rule'></div>")
    return "".join(out)
...
def _css(t) -> str:
    ...
    # --- @page margin boxes for running header/footer ------------------------
    page_boxes = ""
    has_page_field = False
    if t.header:
        font = t.header.font or t.fonts.body
        rule = "border-bottom:1px solid #" + c.rule + ";padding-bottom:4pt;" if t.header.rule else ""
        for box, txt in [("top-left", t.header.left), ("top-center", t.header.center),
                          ("top-right", t.header.right)]:
            if txt:
                page_boxes += (
    ...
    if t.footer:
        font = t.footer.font or t.fonts.body
        rule = "border-top:1px solid #" + c.rule + ";padding-top:4pt;" if t.footer.rule else ""
        for box, txt in [("bottom-left", t.footer.left), ("bottom-center", t.footer.center),
    ...
    if not has_page_field:
        page_boxes += (
            f'    @bottom-center {{ content: counter(page); '
            f'font-family: "{t.fonts.body}"; font-size: 9pt; color: #{c.text}; }}\n'
        )
    # --- page-margin-based rule fallback for engines that don't repeat fixed
    #     elements (e.g. WeasyPrint uses margin boxes, browsers use fixed divs).
    rule_css = ""
    margin_cm = t.page.margin_cm
    if t.header and t.header.rule:
        rule_css += (
            f".mk-hdr-rule{{position:fixed;top:{margin_cm}cm;left:{margin_cm}cm;"
            f"right:{margin_cm}cm;height:0;border-bottom:1px solid #{c.rule};}}\n"
        )
    if t.footer and t.footer.rule:
        rule_css += (
            f".mk-ftr-rule{{position:fixed;bottom:{margin_cm}cm;left:{margin_cm}cm;"
            f"right:{margin_cm}cm;height:0;border-top:1px solid #{c.rule};}}\n"
        )
    # --- assemble ------------------------------------------------------------
    return f"""
@page {{
