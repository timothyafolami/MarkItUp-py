...
@dataclass
class Running:
    """A running header or footer printed on every page (left/center/right).
    Use {page} and {pages} placeholders for the current/total page number,
    e.g. center="Confidential  |  Page {page} of {pages}"."""
    left: str = ""
    center: str = ""
    right: str = ""
    color: str = "555555"
    size_pt: float = 9.0
    rule: bool = True            # divider line under header / above footer
    font: Optional[str] = None   # defaults to the theme body font
...
class Theme:
    ...
    header: Optional[Running] = None
    footer: Optional[Running] = None
    code_size: float = 9.5
    ...
    def from_dict(cls, d: Dict) -> "Theme":
        ...
        header_d = d.get("header")
        footer_d = d.get("footer")
        return cls(
            ...
            header=Running(**header_d) if header_d else None,
            footer=Running(**footer_d) if footer_d else None,
