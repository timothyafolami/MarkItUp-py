...
def render(doc_ir: ir.Document, theme, out_path: str) -> str:
    ...
    if theme.header or theme.footer:
        for section in document.sections:
            _apply_running(section, theme)
    if theme.watermark.enabled:
        for section in document.sections:
            _add_watermark(section, theme.watermark, theme.fonts.heading)
...
def _apply_running(section, theme):
    content_w = section.page_width - section.left_margin - section.right_margin
    if theme.header:
        p = section.header.paragraphs[0]
        _fill_running(p, theme.header, content_w, theme)
        if theme.header.rule:
            _para_edge_border(p, "bottom", theme.colors.rule)
    if theme.footer:
        p = section.footer.paragraphs[0]
        _fill_running(p, theme.footer, content_w, theme)
        if theme.footer.rule:
            _para_edge_border(p, "top", theme.colors.rule)


def _fill_running(p, cfg, content_w, theme):
    # clear any seed text
    for r in list(p.runs):
        r._r.getparent().remove(r._r)
    font = cfg.font or theme.fonts.body
    L, C, R = cfg.left, cfg.center, cfg.right
    multi = sum(bool(x) for x in (L, C, R)) > 1 or (L and R)
    if not multi:
        text = C or R or L
        p.alignment = (WD_ALIGN_PARAGRAPH.CENTER if C else
                       WD_ALIGN_PARAGRAPH.RIGHT if R else WD_ALIGN_PARAGRAPH.LEFT)
        _emit_runs(p, text, cfg, font)
        return
    # 3-part layout via center + right tab stops
    tabs = p.paragraph_format.tab_stops
    tabs.add_tab_stop(Emu(int(content_w / 2)), WD_TAB_ALIGNMENT.CENTER)
    tabs.add_tab_stop(Emu(int(content_w)), WD_TAB_ALIGNMENT.RIGHT)
    _emit_runs(p, L, cfg, font)
    p.add_run("\t")
    _emit_runs(p, C, cfg, font)
    p.add_run("\t")
    _emit_runs(p, R, cfg, font)


def _emit_runs(p, text, cfg, font):
    """Append literal runs, substituting {page}/{pages} with Word fields."""
    if not text:
        return
    for part in re.split(r"(\{page\}|\{pages\})", text):
        if part == "{page}":
            run = _field_run(p, "PAGE")
        elif part == "{pages}":
            run = _field_run(p, "NUMPAGES")
        elif part:
            run = p.add_run(part)
        else:
            continue
...
def _para_edge_border(p, edge, color):
